    ...
    using (store)
    {
        ...
        if (kind is not "member" && !preferMember)
        {
            ...
            if (typeDetail is not null)
            {
                var body = format switch
                {
                    "json" => CompactExporter.TypeDetailToJson(typeDetail),
                    "md" or "markdown" => CompactExporter.TypeDetailToMarkdown(typeDetail),
                    _ => throw new InvalidOperationException($"Unknown format: {format}")
                };
                return WriteOutput(body, outFile);
            }
        ...
        {
            "json" => CompactExporter.MemberDetailToJson(member),
            "md" or "markdown" => CompactExporter.MemberDetailToMarkdown(member),
            _ => throw new InvalidOperationException($"Unknown format: {format}")
        };
...
var callersArg = new Argument<string>("method")
{
    Description = "Method name, Type.Method, or method:Id."
};
var callersUpdate = new Option<bool>("--update")
{
    Description = "Persist callers into members.consumers_json."
};

var callersCmd = new Command("callers", "Find method callers via SymbolFinder (on-demand, scoped).")
{
    callersArg,
    dbOption,
    formatOption,
...
callersCmd.SetAction(async (parseResult, ct) =>
{
    ...
    try
    {
        using (store)
        {
            ...
            var member = store.GetMemberDetail(methodName);
            if (member is null)
            {
                Console.Error.WriteLine($"Member not found in index: {methodName}. Index first, or use Type.Method.");
                return 3;
            }
            ...
            {
                Console.Error.WriteLine("Solution path missing from index.");
                return 1;
            }

            Console.Error.WriteLine($"Finding callers of {member.ParentTypeFullName}.{member.Name}...");
            using var loader = WorkspaceLoader.Create();
            var solution = await loader.OpenAsync(status.SolutionPath, ct).ConfigureAwait(false);
            var symbol = await MethodCallersExtractor.FindMethodSymbolAsync(
                solution, member.Id, member.ParentTypeFullName, ct).ConfigureAwait(false);
            if (symbol is null)
            {
                // fallback search by display name
                symbol = await MethodCallersExtractor.FindMethodSymbolAsync(
                    solution, member.Name, member.ParentTypeFullName, ct).ConfigureAwait(false);
            }

            if (symbol is null)
            {
                Console.Error.WriteLine("Could not resolve method symbol in workspace.");
                return 1;
            }

            var extractor = new MethodCallersExtractor();
            var callers = await extractor.FindCallersAsync(solution, symbol, ct).ConfigureAwait(false);

            if (update)
            {
                store.SaveMemberConsumers(member.Id, callers);
                Console.Error.WriteLine($"Updated consumers_json ({callers.Count} callers).");
            }

            var label = $"{member.ParentTypeFullName}.{member.Name}";
            string body = format switch
            {
                "json" => System.Text.Json.JsonSerializer.Serialize(new
                {
                    ...
                    count = callers.Count,
                    callers,
                    lines = new { start = member.StartLine, end = member.EndLine, count = member.LineCount },
                    file = member.RelativePath,
                    updated = update
                }),
                "md" or "markdown" => CompactExporter.CallersToMarkdown(label, callers),
                _ => throw new InvalidOperationException($"Unknown format: {format}")
            };
